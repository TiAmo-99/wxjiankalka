...
import { createClientHeartbeat, serializeMessage } from './protocol.js'
import {
        ...
        dispatchJsonMessage(json, {
          onReply: (msg) => {
            ...
            ensureTransport()
              .sendJson(msg)
              .catch((e) => {
                appendCommLog({ dir: 'TX', mode: 'warn', text: `心跳应答失败: ${e.message}` })
              })
          }
        })
      },
...
export function startHeartbeat() {
  ...
  const tick = async () => {
    ...
    try {
      heartbeatSeq += 1
      const t = ensureTransport()
      await t.sendJson(createClientHeartbeat(heartbeatSeq))
    } catch (e) {
