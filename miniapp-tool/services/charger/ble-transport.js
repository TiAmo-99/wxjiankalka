...
export function createBleTransport(hooks = {}) {
  ...
  async function resolveServiceAndChars(services) {
    ...
    const tryOrder = []
    if (config.serviceId) {
      const hit = list.find((s) => uuidMatch(s.uuid, config.serviceId))
      if (hit) tryOrder.push(hit)
    }
    for (const s of list) {
      if (!tryOrder.some((x) => x.uuid === s.uuid)) tryOrder.push(s)
    }
  ...
  async function writeBuffer(buffer) {
    ...
    const arr = new Uint8Array(buffer)
    const chunkSize = Math.max(8, Number(config.writeChunkSize) || 20)
  ...
  async function sendText(text) {
    ...
    appendCommLog({ dir: 'TX', mode: 'hex', text: hexPreview(buf) })
    appendCommLog({ dir: 'TX', mode: 'text', text })
    await writeBuffer(buf)
  }

  async function sendJson(obj, { appendCrlf } = {}) {
