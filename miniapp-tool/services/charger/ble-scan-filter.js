/**
 * 扫描列表：显示全部 BLE 设备（未命名用 MAC 后缀），不做信号强度过滤
 */
...
export function displayBleDevices(devices) {
  ...
  for (const raw of devices) {
    ...
    if (d.name) {
      named++
      list.push({ deviceId: d.deviceId, name: d.name, localName: d.localName, RSSI: d.RSSI, isUnnamed: false })
    } else {
      unnamed++
      list.push({
        deviceId: d.deviceId,
